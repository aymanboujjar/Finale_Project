...
<body class="bg-gradient-to-br from-purple-50 via-white to-blue-50 min-h-screen font-sans">
    ...
    <div id="mobile-menu" class="fixed inset-0 bg-white z-40 transform translate-x-full transition-transform duration-300 md:hidden">
        <div class="flex flex-col items-center justify-center h-full space-y-8">
            @if (Route::has('login'))
                @auth
                    <a href="{{ url('/profile') }}" class="text-xl text-gray-600 hover:text-indigo-600">Profile</a>
                @else
                    <a href="{{ route('login') }}" class="text-xl text-gray-600 hover:text-indigo-600">Log in</a>
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="text-xl bg-indigo-600 text-white px-8 py-3 rounded-lg hover:bg-indigo-700 transition-colors">Join us for free</a>
                    @endif
                @endauth
            @endif
        </div>
    ...
    <!-- Main Content -->
    <div class="min-h-screen pt-20 md:pt-32 pb-20 px-4">
        <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center">
            <!-- Left Side - Animation -->
            <div class="flex flex-col items-center justify-center order-2 lg:order-1">
                <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-6 text-center">Start Your Learning Journey</h2>
                <p class="text-base md:text-lg text-gray-600 mb-8 text-center max-w-md px-4">Join our community of learners and unlock your potential with expert-led courses and personalized learning paths.</p>
                <div class="relative w-full max-w-lg">
                    ...
                    <lottie-player
                        src="https://lottie.host/2c99c8b1-2f93-4df9-b3c9-c5b0a1f8e987/qGBWzfTrVa.json"
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden p-4 md:p-8 order-1 lg:order-2">
                <div class="text-center mb-6 md:mb-8">
                    <h1 class="text-xl md:text-2xl font-extrabold text-indigo-600">Create Your Account</h1>
                </div>

                <form method="POST" action="{{ route('register') }}" enctype="multipart/form-data" class="space-y-4 md:space-y-6">
                    @csrf
                    ...
                    <div class="flex flex-col md:flex-row items-center justify-between gap-4 mt-6">
                        ...
                        <button type="submit" class="w-full md:w-auto bg-indigo-600 text-white px-8 py-3 rounded-lg font-medium hover:bg-indigo-700 transition-colors">
                            {{ __('Register') }}
                        </button>
                    </div>
                </form>
